...
generated_toc = {
  generate: function() {
    ...
    generate_from = '2';
    generate_for = '';
    tocparent = document.getElementById('toc');
    if (tocparent) {
      ...
      for (var i=0; i<classes.length; i++) {
        ...
        if (classes[i].match(/^generate_from_h[1-4]$/)) {
    ...
    for (var i=0; i<headings.length; i++) {
	  ...
	  //on click go back to toc.
	  this_head_el.addEventListener('click',function(){ window.location.href = "#"+'toc'});
      if (!this_head_el.id) {
        // if heading doesn't have an ID, give it one
        this_head_el.id = 'toc_'+(i+1)+"_"+this_head_el.innerHTML;
