...
class IdMapperHttpHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path=="/map/ids":
            if 'solonoidId' in body and 'relayAddress' in body:
                try:
                    idm = IdMapper()
                    # TODO: why does it only work when I instantiate idmapper here?
                    #TODO somehow have global id_dict
                    idm.setRelayAddress(body['solonoidId'], body['relayAddress'])
                    self.send_response(200)

                except:
                    self.send_response(500, "Error: something happened. Maybe solonoidid is incorrect")
            else:
                print "poorly formed request"
                self.send_response(422, "Malformed request. You must include a relayAddress and solonoidId.")
                return
